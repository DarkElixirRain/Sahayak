...
def check_and_start_redis():
    """Check if any Redis container is running, start if necessary."""
    try:
        # Check if ANY Redis container is running on port 6379
        check_running_cmd = ["docker", "ps", "-q", "-f", "ancestor=redis"]
        running_containers = subprocess.check_output(check_running_cmd).strip()

        if running_containers:
            # Get the name of the running container
            get_name_cmd = ["docker", "ps", "--format", "{{.Names}}", "-f", "ancestor=redis"]
            container_name = subprocess.check_output(get_name_cmd).decode().strip().split('\n')[0]
            logging.info(f"Redis container ({container_name}) is already running.")
            return

        # Check if morphik-redis or morphik-core-redis-1 exists but is stopped
        for container_name in ["morphik-core-redis-1", "morphik-redis"]:
            check_exists_cmd = ["docker", "ps", "-a", "-q", "-f", f"name={container_name}"]
            existing_container = subprocess.check_output(check_exists_cmd).strip()

            if existing_container:
                logging.info(f"Starting existing Redis container ({container_name})...")
                subprocess.run(["docker", "start", container_name], check=True, capture_output=True)
                logging.info("Redis container started.")
                return

        # If no container exists, use docker-compose
        logging.info("No Redis container found. Starting via docker-compose...")
        subprocess.run(
            ["docker", "compose", "up", "-d", "redis"],
            check=True,
            capture_output=True,
        )
        logging.info("Redis container created and started via docker-compose.")
