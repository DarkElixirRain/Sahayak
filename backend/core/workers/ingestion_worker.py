...
# from opentelemetry.trace import Status, StatusCode, get_current_span  # Disabled - opentelemetry not available
from sqlalchemy import text

from core.config import get_settings
from core.database.postgres_database import PostgresDatabase
# from core.embedding.colpali_api_embedding_model import ColpaliApiEmbeddingModel  # Disabled - requires colpali_engine
# from core.embedding.colpali_embedding_model import ColpaliEmbeddingModel  # Disabled - requires colpali_engine
from core.embedding.litellm_embedding import LiteLLMEmbeddingModel
from core.embedding.ollama_embedding import OllamaEmbeddingModel
...
# from core.storage.s3_storage import S3Storage  # Disabled - requires boto3
...
# from core.vector_store.dual_multivector_store import DualMultiVectorStore  # Disabled - requires FastMultiVectorStore
# from core.vector_store.fast_multivector_store import FastMultiVectorStore  # Disabled - requires torch/colpali
# from core.vector_store.multi_vector_store import MultiVectorStore  # Disabled - requires torch/S3Storage
from core.vector_store.pgvector_store import PGVectorStore

# Enterprise routing helpers (optional - fallback to default if not available)
try:
    from ee.db_router import get_database_for_app, get_vector_store_for_app
except ModuleNotFoundError:
    # Community edition - use global singleton database and vector store
    async def get_database_for_app(app_id):
        from core.services_init import database
        return database
    
    async def get_vector_store_for_app(app_id):
        from core.services_init import vector_store
        return vector_store
...
async def _get_worker_colpali_store(database: PostgresDatabase) -> Optional[BaseVectorStore]:
    ...
    # ColPali multivector stores disabled - not available without torch/colpali dependencies
    raise ValueError("ColPali is disabled in simplified version")
        ...
        async with telemetry.track_operation(
            ...
            if should_create_image_chunks:
                try:
                    ...
                    # Opentelemetry tracking disabled
            ...
            if using_colpali:
                # ColPali API check disabled - class not imported
                is_api = False  # isinstance(ingestion_service.colpali_embedding_model, ColpaliApiEmbeddingModel)
...
async def startup(ctx):
    ...
    if settings.STORAGE_PROVIDER == "local":
        ...
        # S3 storage disabled - requires boto3, fallback to local
        logger.warning("AWS S3 storage not supported in simplified version, using local storage")
        storage = LocalStorage(storage_path=settings.STORAGE_PATH)
        # storage = S3Storage(
    ...
    if embedding_model_config.get("provider") == "ollama_direct":
        # Use direct Ollama implementation
        model_name = embedding_model_config.get("model_name", "nomic-embed-text")
        api_base = embedding_model_config.get("api_base", "http://localhost:11434")
        embedding_model = OllamaEmbeddingModel(model_name=model_name, api_base=api_base)
        logger.info(f"Initialized Ollama direct embedding model with model: {model_name}")
    else:
    ...
    if not settings.ENABLE_COLPALI:
        ...
        # ColPali disabled in simplified version - requires torch/colpali_engine
        logger.warning("ColPali is not supported in simplified version (mode was set to '%s')", settings.COLPALI_MODE)
