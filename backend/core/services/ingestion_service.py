...
import arq
import fitz  # PyMuPDF
# import pdf2image  # Fallback for PDF processing - not available in simplified version
...
class IngestionService:
    ...
    def _process_pdf_for_colpali(self, file_content: bytes) -> List[Chunk]:
        ...
        try:
            ...
            # pdf2image fallback disabled in simplified version
            logger.error(f"PyMuPDF failed and pdf2image fallback is not available: {e}")
            raise PdfConversionError(f"Unable to convert PDF to images: {e}") from e
    ...
    def _convert_office_to_images(
        ...
        try:
            ...
            try:
                ...
                # pdf2image fallback disabled in simplified version
                logger.warning(f"PyMuPDF failed for {doc_type} ({pymupdf_error}), pdf2image fallback not available")
                logger.info(f"Falling back to text extraction for {doc_type}")
                return [
