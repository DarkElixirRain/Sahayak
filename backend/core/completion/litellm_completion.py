...
class LiteLLMCompletionModel(BaseCompletionModel):
    ...
    async def complete(self, request: CompletionRequest) -> Union[CompletionResponse, AsyncGenerator[str, None]]:
        ...
        # Standard completion (if structured output is not requested or failed)
        if is_ollama:
            fallback_response = await self._handle_standard_ollama(user_content, ollama_image_data, request, history_messages)
        else:
            fallback_response = await self._handle_standard_litellm(user_content, image_urls, request, history_messages, model_config)

        # If we had requested structured output but had to fall back, we MUST wrap it in JSON
        if request.response_schema is not None and not structured_output:
            logger.warning("Wrapping fallback text in structured JSON format to satisfy frontend contract")
            
            wrapped_completion = {
                "response_type": "legal_clarification",
                "message": "तपाईंको प्रश्नको उत्तर तयार गर्दा समस्या भयो। कृपया प्रश्नलाई अलि स्पष्ट रूपमा लेख्नुहोस्।"
            }
            fallback_response.completion = wrapped_completion
