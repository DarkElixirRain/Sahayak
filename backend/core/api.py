...
import jwt
import requests
# DISABLED: Monitoring tools removed for simplified deployment
# import sentry_sdk
import tomli
from fastapi import Depends, FastAPI, Form, Header, HTTPException, Query, Request
from fastapi.middleware.cors import CORSMiddleware  # Import CORSMiddleware
from fastapi.responses import JSONResponse, StreamingResponse
# DISABLED: OpenTelemetry removed for simplified deployment
...
# DISABLED: Profiling middleware removed for simplified deployment
...
# DISABLED: Unused routers for simplified deployment
...
from core.routes.audio import DUMMY_RESPONSE_AUDIO_URL
from core.routes.audio import router as audio_router
...
# ---------------------------------------------------------------------------
# Initialize Sentry - DISABLED for simplified deployment
# ---------------------------------------------------------------------------
...
#     )
# else:
#     logger.info("SENTRY_DSN is not set, skipping Sentry initialization")

# ---------------------------------------------------------------------------
# Application instance & core initialisation (moved lifespan, rest unchanged)
# ---------------------------------------------------------------------------

app = FastAPI(title="Sahayak API", description="Sahayak legal assistance backend", lifespan=lifespan)

# --------------------------------------------------------
# Optional per-request profiler - DISABLED for simplified deployment
# --------------------------------------------------------
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"detail": "सर्भरमा आन्तरिक समस्या देखियो।"},
    )

# Initialise telemetry service
telemetry = TelemetryService()

# OpenTelemetry instrumentation - DISABLED for simplified deployment
...
# Register audio router (dummy response audio for the MVP voice flow)
...
try:
    ...
    logger.debug("ModuleNotFoundError: %s", exc)
except ImportError as exc:
    logger.warning("Failed to import init_app from ee.routers: %s", exc)
...
async def query_completion(
    ...
    # Short-circuit for casual greetings
    casual_greetings = {
        "hi", "hello", "namaste", "नमस्कार", "good morning", 
        "thank you", "thanks", "bye", "how are you?", "how are you",
        "नमस्ते", "धन्यवाद"
    }
    query_lower = request.query.strip().lower()
    if query_lower in casual_greetings:
        if query_lower in ["hi", "hello", "namaste", "नमस्कार", "नमस्ते", "good morning"]:
            message = "नमस्कार! 👋 म Sahayak हुँ। तपाईंलाई नेपालको कानुनसम्बन्धी कुनै प्रश्न वा समस्या छ भने बताउनुहोस्।"
        elif query_lower in ["thank you", "thanks", "धन्यवाद"]:
            message = "स्वागत छ! 😊 अरू कुनै कानुनी प्रश्न भए सोध्न सक्नुहुन्छ।"
        elif query_lower in ["bye"]:
            message = "बिदा! 👋 अरू कुनै सहयोग चाहिएमा सम्झनुहोला।"
        else:
            message = "नमस्कार! म Sahayak हुँ। म तपाईंलाई कसरी सहयोग गर्न सक्छु?"
        ...
        # We simulate the exact response format expected by Flutter
        return CompletionResponse(
            completion=json.dumps(casual_response),
            usage={"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0},
            sources=[],
            audio_url=DUMMY_RESPONSE_AUDIO_URL,
        )
    ...
    try:
        ...
        if request.chat_id:
            ...
            # --- DEMO/DEV CONTEXT GUARD ---
            # Keep only the last MAX_HISTORY_TURNS complete turns (user+assistant pairs)
            # plus the current user message to prevent Groq context-window overflow (413).
            # In a multi-turn chat, full history grows quickly and exceeds the model's limit.
            MAX_HISTORY_TURNS = 6          # 6 turns = 12 messages + current = 13 total max
            max_messages = MAX_HISTORY_TURNS * 2 + 1   # always include current user msg at end
            if len(history) > max_messages:
                # Keep the most recent messages; always preserve the last entry (current user msg)
                history = history[-max_messages:]
                logger.info(f"History truncated to last {max_messages} messages to prevent context overflow")
        ...
        # Inject Sahayak system prompt if no overrides
        if not request.prompt_overrides:
            from core.models.prompts import QueryPromptOverrides, QueryPromptOverride
            from core.services.sahayak_prompts import SAHAYAK_SYSTEM_PROMPT
            request.prompt_overrides = QueryPromptOverrides(query=QueryPromptOverride(system_prompt=SAHAYAK_SYSTEM_PROMPT))
        ...
        try:
            ...
            if "invalid api key" in err_msg or "authentication" in err_msg or "401" in err_msg:
                raise HTTPException(status_code=401, detail="Sahayak सेवा अहिले उपलब्ध छैन। (Auth error)")
            elif "413" in err_msg or "too large" in err_msg:
                raise HTTPException(status_code=413, detail="कुराकानीको सन्देश धेरै लामो भयो। कृपया केही पुरानो सन्देश हटाएर वा नयाँ कुराकानी सुरु गरेर प्रयास गर्नुहोस्।")
            elif "rate limit" in err_msg or "rate_limit" in err_msg or "429" in err_msg or "ratelimit" in err_type:
                raise HTTPException(status_code=429, detail="इन्टरनेट वा सर्भरमा समस्या देखियो। कृपया फेरि प्रयास गर्नुहोस्।")
            elif "timeout" in err_msg or "504" in err_msg:
                raise HTTPException(status_code=504, detail="सर्भरले समयमा प्रतिक्रिया दिन सकेन। कृपया फेरि प्रयास गर्नुहोस्।")
            elif "connection" in err_msg or "unavailable" in err_msg or "502" in err_msg:
                raise HTTPException(status_code=502, detail="Sahayak सेवा अहिले उपलब्ध छैन। कृपया केही समयपछि फेरि प्रयास गर्नुहोस्।")
            else:
                raise HTTPException(status_code=500, detail="सर्भरमा आन्तरिक समस्या देखियो।")
        ...
        if request.stream_response:
            ...
            # Attach the MVP dummy audio reference. The same pre-recorded
            # response is returned for every query; real TTS replaces this.
            response.audio_url = DUMMY_RESPONSE_AUDIO_URL
