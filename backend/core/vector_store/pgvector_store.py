...
class PGVectorStore(BaseVectorStore):
    ...
    async def query_similar(
        ...
        """
        try:
            # WORKAROUND: When multiple document IDs are provided, query each separately
            # and merge results. This avoids a bug with pgvector + SQLAlchemy + asyncpg
            # when using IN clauses with vector similarity queries.
            if doc_ids and len(doc_ids) > 1:
                logger.debug(f"Querying {len(doc_ids)} documents separately and merging results")
                all_chunks = []
                
                # Query each document separately
                for doc_id in doc_ids:
                    chunks = await self.query_similar(
                        query_embedding,
                        k=k * 2,  # Get more from each to ensure we have enough after merging
                        doc_ids=[doc_id],
                        app_id=app_id,
            ...
            # Original implementation for single document or no filter
            async with self.get_session_with_retry() as session:
                # Build query with cosine distance calculation
                distance = VectorEmbedding.embedding.op("<=>")(query_embedding)
                query = select(VectorEmbedding, distance).order_by(distance)

                if doc_ids:
                    logger.debug(f"Filtering by document ID: {doc_ids[0]}")
                    query = query.filter(VectorEmbedding.document_id == doc_ids[0])
                else:
                    logger.debug("No document filter applied")

                query = query.limit(k)
                result = await session.execute(query)
                embeddings = result.all()
                logger.debug(f"Vector query returned {len(embeddings)} results")
