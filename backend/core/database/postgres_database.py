...
class PostgresDatabase:
    ...
    async def find_authorized_and_filtered_documents(
        ...
        try:
            async with self.async_session() as session:
                # Extract document ID filters before other processing
                doc_id_filter = None
                modified_filters = filters
                
                if filters:
                    # Check for document ID filters
                    doc_id_single = filters.get('external_id') or filters.get('document_id')
                    doc_ids_list = filters.get('external_ids') or filters.get('document_ids')
                    
                    # Merge single and list filters
                    if doc_id_single and doc_ids_list:
                        # Both provided - merge them
                        if isinstance(doc_id_single, str):
                            doc_id_filter = [doc_id_single]
                        else:
                            doc_id_filter = doc_id_single if isinstance(doc_id_single, list) else [doc_id_single]
                        
                        if isinstance(doc_ids_list, list):
                            doc_id_filter.extend(doc_ids_list)
                    elif doc_id_single:
                        doc_id_filter = doc_id_single
                    elif doc_ids_list:
                        doc_id_filter = doc_ids_list
                    
                    # Remove document ID keys from filters to avoid double-processing
                    if doc_id_filter is not None:
                        modified_filters = {k: v for k, v in filters.items() 
                                          if k not in ('external_id', 'document_id', 'external_ids', 'document_ids')}
                
                # Build query
                access_filter = self._build_access_filter_optimized(auth)
                metadata_filter = self._build_metadata_filter(modified_filters)
                system_metadata_filter = self._build_system_metadata_filter_optimized(system_filters)
                filter_params = self._build_filter_params(auth, system_filters)
                ...
                logger.debug(f"Document ID filter: {doc_id_filter}")
                ...
                if doc_id_filter is not None:
                    if isinstance(doc_id_filter, str):
                        # Single document ID
                        where_clauses.append("external_id = :doc_id_filter")
                        filter_params['doc_id_filter'] = doc_id_filter
                    elif isinstance(doc_id_filter, list) and len(doc_id_filter) > 0:
                        # Multiple document IDs
                        doc_id_clauses = []
                        for idx, doc_id in enumerate(doc_id_filter):
                            param_name = f"doc_id_{idx}"
                            doc_id_clauses.append(f"external_id = :{param_name}")
                            filter_params[param_name] = doc_id
                        where_clauses.append("(" + " OR ".join(doc_id_clauses) + ")")
