...
from core.completion.gemini_completion import GeminiCompletionModel
...
# from core.reranker.flag_reranker import FlagReranker  # Disabled - requires FlagEmbedding
...
match settings.STORAGE_PROVIDER:
    ...
    case "aws-s3":
        # S3 storage disabled - requires boto3 package
        logger.warning("AWS S3 storage not supported in simplified version, falling back to local storage")
        storage = LocalStorage(storage_path=settings.STORAGE_PATH)
...
# Check if using direct Ollama provider for embeddings
...
if embedding_model_config.get("provider") == "ollama_direct":
    ...
    logger.info("Initialized Ollama direct embedding model with model: %s", model_name)
else:
...
# Check if using direct Gemini provider
model_config = settings.REGISTERED_MODELS.get(settings.COMPLETION_MODEL, {})
if model_config.get("provider") == "gemini_direct":
    # Use direct Gemini implementation
    model_name = model_config.get("model_name", "gemini-1.5-flash")
    completion_model = GeminiCompletionModel(model_name=model_name)
    logger.info("Initialized Gemini direct completion model with model: %s", model_name)
else:
    # Use LiteLLM for other providers
...
# Reranker disabled - FlagReranker requires FlagEmbedding package
reranker = None
...
if not settings.ENABLE_COLPALI:
    ...
    match settings.COLPALI_MODE:
        case "off":
            ...
            colpali_vector_store = None
        case "local":
            # ColPali local mode disabled - requires torch/colpali_engine
            logger.warning("ColPali local mode is not supported in simplified version")
            colpali_embedding_model = None
            colpali_vector_store = None
            ...
            #     )
        case "api":
            # ColPali API mode disabled - requires ColpaliApiEmbeddingModel
            logger.warning("ColPali API mode is not supported in simplified version")
            colpali_embedding_model = None
            colpali_vector_store = None
