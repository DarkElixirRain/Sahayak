...
# from core.parser.video.parse_video import VideoParser, load_config  # Disabled - requires assemblyai
...
class ContextualChunker(BaseChunker):
    ...
    def __init__(self, chunk_size: int, chunk_overlap: int, anthropic_api_key: str):
        ...
        # Default model since load_config is disabled
        self.model_key = "claude_sonnet"
...
class MorphikParser(BaseParser):
    ...
    # Disabled - requires assemblyai package
    ...
    async def parse_file_to_text(self, file: bytes, filename: str) -> Tuple[Dict[str, Any], str]:
        ...
        if self._is_video_file(file, filename):
            # Video parsing disabled - requires assemblyai package
            raise ValueError("Video parsing is not enabled in this simplified version")
